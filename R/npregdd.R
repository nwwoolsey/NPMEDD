...
npregdd<-function(h,method,x,y,delta,error,sigmau,rep){
  ...
  if(method=="deconv"){
    g1<-sin(y)
    g2<-cos(y)
    m1<-meanreg(g1,x,h,xgrid=delta,method="DFC",sig=sigmau,error=error)$yhat
    m2<-meanreg(g2,x,h,xgrid=delta,method="DFC",sig=sigmau,error=error)$yhat
    m<-atan(m1/m2)
    return(m)
  }
  if(method=="comp"){
    ...
    errs<-function(s,h){
      t<-matrix(s,ncol=rep,nrow=length(y))+1i*sigmau*matrix(rnorm(length(y)*rep,mean=0,sd=1),ncol=rep,nrow=length(y))
  ...
  if(method=="onestep"){
    g1<-sin(y)
    g2<-cos(y)
    m1<-meanreg(g1,x,h,xgrid=delta,method="HZ",sig=sigmau,error=error)$yhat
    m2<-meanreg(g2,x,h,xgrid=delta,method="HZ",sig=sigmau,error=error)$yhat
    m<-atan(m1/m2)
    return(m)
  }
}
